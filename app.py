...
def init_db():
    ...
    conn.executescript("""
        ...
        CREATE TABLE IF NOT EXISTS mock_tests (
            ...
            profile_id INTEGER NOT NULL REFERENCES profiles(id),
            date TEXT NOT NULL,
            english_qs INTEGER, english_score REAL,
            gk_qs INTEGER, gk_score REAL,
            legal_qs INTEGER, legal_score REAL,
            logical_qs INTEGER, logical_score REAL,
            quant_qs INTEGER, quant_score REAL
        );
...
def mock_total_score(row):
    """Add up the five section scores for one mock test row."""
    fields = ["english_score", "gk_score", "legal_score", "logical_score", "quant_score"]
    return sum(row[f] or 0 for f in fields)


def mock_total_questions(row):
    fields = ["english_qs", "gk_qs", "legal_qs", "logical_qs", "quant_qs"]
    return sum(row[f] or 0 for f in fields)
...
def mocks():
    ...
    if request.method == "POST":
        ...
        for s in SUBJECTS:
            values.append(request.form.get(f"{s['id']}_qs", type=int))
            values.append(request.form.get(f"{s['id']}_score", type=float))
        db.execute(
            """INSERT INTO mock_tests
               (profile_id, date, english_qs, english_score, gk_qs, gk_score,
                legal_qs, legal_score, logical_qs, logical_score, quant_qs, quant_score)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            values,
        )
        db.commit()
...
def generate_charts(profile_id):
    ...
    latest = rows[-1]
    score_fields = ["english_score", "gk_score", "legal_score", "logical_score", "quant_score"]
    labels = [s["roman"] for s in SUBJECTS]
    values = [latest[f] or 0 for f in score_fields]
